...
const char *command_line(int argc, const char **argv) {
	...
	for (i = 1; i < argc; i++) {
		if (argv[i][0] == '-') {
			...
			else fprintf(stderr,"ignoring unrecognized option \"%s\"\n",argv[i]);
		}
		else if (!wavname) {
			wavname = argv[i];
		}
	...
	if (winfun) {
		if (strncmp(winfun,"custom",6)==0 && sscanf(winfun,"custom=%lf,%lf,%lf,%lf",
...
Wave *read_wave(const char *fname) {
	...
	w->d = (double *) malloc(wi->frames*sizeof(double));
	sf_count_t n = sf_readf_double(wf,w->d,wi->frames);
	w->samples = n;
		...
		while (previewing) {
			...
			for (i = 1; i < fft->ts; i++) {
				for (f = 0, j = 0; j < fft->fs; j++)
					if (fft->amp[i][j] > fft->amp[i][f] && fft->freq[j] > hipass)
							f = j;
