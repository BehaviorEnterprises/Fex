...
typedef enum {
	FexShowHud, FexShowHudResizing,
	FexShowOverlay, FexShowOverlayResizing,
	FexConfLAST,
} FexConfVar;

extern fex_t fex_create(const char *);
extern fex_t fex_destroy(fex_t);
extern int fex_floor(fex_t, float);
extern int fex_get_conf(fex_t,FexConfVar);
extern int *fex_get_path(fex_t);
extern unsigned char *fex_get_spectrogram(fex_t, int *, int *);
extern int fex_set_con(fex_t,FexConfVar,int);
